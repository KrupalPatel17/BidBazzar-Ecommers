...
session_start();
include("connect.php");
// $sql = "SELECT * FROM tbl_auction WHERE (a_id LIKE '%{$search_val}%' OR p_name LIKE '%{$search_val}%')";
...
// $aresult = mysqli_query($connect, $select);
// $uresult = mysqli_fetch_assoc($aresult);
// $vname = $uresult['vuser_name'];

$sql = "SELECT tbl_auction.*, tbl_vender.vuser_name 
        FROM tbl_auction
        INNER JOIN tbl_vender ON tbl_auction.v_id = tbl_vender.vid 
        WHERE (a_id LIKE '%{$search_val}%' OR p_name LIKE '%{$search_val}%')";
